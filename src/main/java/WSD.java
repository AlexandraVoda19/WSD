...
public class WSD {
    ...
    public WSD(String targetWord, String inputPhrase, WNSense sense) throws JWNLException, FileNotFoundException {
...
//        Pair<String, String> gloss_hypo = new Pair(GLOSS_REL, PointerType.HYPONYM.getLabel());
        ...
        Pair<String, String> attr_gloss = new Pair(PointerType.ATTRIBUTE.getLabel(), GLOSS_REL);
        Pair<String, String> gloss_attr = new Pair(GLOSS_REL, PointerType.ATTRIBUTE.getLabel());
        Pair<String, String> also_gloss = new Pair(PointerType.SEE_ALSO.getLabel(), GLOSS_REL);
        Pair<String, String> gloss_also = new Pair(GLOSS_REL, PointerType.SEE_ALSO.getLabel());
...
//        relpairs.add(gloss_hypo);
        ...
        relpairs.add(attr_gloss);
        relpairs.add(gloss_attr);
//        relpairs.add(also_gloss);
//        relpairs.add(gloss_also);
    }

    public boolean isWordInWN(String word) throws JWNLException {
    ...
    public void initWindowWords() throws JWNLException {
        ...
        while( targetWordPosition + i < sInputPhrase.size()
            ...
            if( isWordInWN(sInputPhrase.get(targetWordPosition + i)) ){
        ...
        while( leftWords.size() < WINDOW_SIZE && targetWordPosition + i < sInputPhrase.size() && (rightWords.size() + leftWords.size()) < WINDOW_SIZE * 2 ){
            if( isWordInWN(sInputPhrase.get(targetWordPosition + i)) ){
                rightWords.add(sInputPhrase.get(targetWordPosition + i));
            }
            i++;
        ...
        while( rightWords.size() < WINDOW_SIZE && targetWordPosition - i >= 0 && (rightWords.size() + leftWords.size()) < WINDOW_SIZE * 2 ){
            if( isWordInWN(sInputPhrase.get(targetWordPosition - i)) ){
                leftWords.add(sInputPhrase.get(targetWordPosition - i));
            }
            i++;
...
//                            sb.append(str1.substring(lastSubsBegin, i + 1));
    ...
    public Integer calculateScore(ArrayList<String> s1, ArrayList<String> s2){
        ...
        while( longestMatch.size() > 0 ){
            ...
            s1.removeAll(s1.subList(Collections.indexOfSubList(s1, longestMatch), Collections.indexOfSubList(s1, longestMatch) + longestMatch.size()));
            s2.removeAll(s2.subList(Collections.indexOfSubList(s2, longestMatch), Collections.indexOfSubList(s2, longestMatch) + longestMatch.size()));
...
//            return lemmatized;
    ...
    private Integer calculateSenseScore(Synset sense) throws JWNLException, FileNotFoundException {
        Integer score = 0;
        ...
        for( String windowWord:windowWords ){
            for ( POS pos : POS.getAllPOS() ) {
                ...
                if(WNwindowWord != null) {
                    for( Synset contextWordSense:WNwindowWord.getSenses() ) {
    ...
    public Long pickCorrectSense() throws JWNLException, FileNotFoundException {
        ...
        for ( POS pos : POS.getAllPOS() ) {
            ...
            if ( WNtarget != null ) {
                for( Synset sense : WNtarget.getSenses() ){
                    Integer senseScore = calculateSenseScore(sense);
